...
async function handleSearch(chatId, query) {
    ...
    try {
        ...
        const keyboard = results.items.map((item, i) => [
            {
                text: item.filesize
...
async function main() {
    ...
    // Progress updates
    await bus.subscribe('download:progress', async (data) => {
    ...
    // Download done
    await bus.subscribe('download:done', async (data) => {
        const chatId = data.chatId.toString();
        const msgId = progressMessages.get(chatId);
        if (msgId) {
        ...
        try {
            ...
            fs.unlink(data.filePath, (err) => {
                if (err)
                    console.error('[BOT] Failed to delete file:', err);
                else
                    console.log('[BOT] Cleaned up:', data.filePath);
            });
        }
    });
    // Message handler
